...
class DatabaseService {
  ...
  Future<void> _createTables(Database db, int version) async {
    try {
      ...
      await db.execute(
        "CREATE TABLE cycles (id INTEGER PRIMARY KEY, start_date DATE, end_date DATE, user_id INTEGER, flow_level TEXT, FOREIGN KEY (user_id) REFERENCES users(id))",
      );
      await db.execute(
  ...
  Future<void> insertUser(Map<String, dynamic> userData) async {
    ...
    try {
      ...
      _logger.info('User inserted successfully with name: ${userData['name']}');
    } catch (e, stackTrace) {
  ...
  Future<bool> verifyUserPassword(String userId, String providedPassword) async {
    ...
    if (result.isNotEmpty) {
      ...
      if (storedHash != null) {
        _logger.info('Verifying password for user ID: $userId with hash: $storedHash');
